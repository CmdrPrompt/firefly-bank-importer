...
Covers _log_tx_result, create_transaction, and _post_transfer logging the
Firefly account name instead of the numeric account ID, with a fallback to
the numeric ID when the name cannot be resolved.
"""
...
class TestLogTxResultAccountName:
    def test_logs_account_name_line(self, caplog: pytest.LogCaptureFixture) -> None:
        with caplog.at_level(logging.INFO):
            _log_tx_result("withdrawal", 69.0, "2025-06-25", "MCDJARFALLAS/25-06-24", "SEB Lönekonto Thomas")
        assert any(
...
class TestCreateTransactionAccountName:
    ...
    def test_falls_back_to_numeric_id_when_name_omitted(self, caplog: pytest.LogCaptureFixture) -> None:
        client = make_client()
        with caplog.at_level(logging.INFO):
            create_transaction(client, "2025-06-25", "X", "-10.00", 99)
...
class TestPostTransferAccountNames:
    ...
    def test_falls_back_to_numeric_ids_when_names_omitted(self, caplog: pytest.LogCaptureFixture) -> None:
        client = make_client()
        with caplog.at_level(logging.INFO):
            _post_transfer(client, self._payload(), dry_run=False)
