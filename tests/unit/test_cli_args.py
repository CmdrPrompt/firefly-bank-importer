...
class TestParseCLIArgsValidInput:
    def test_folder_only(self) -> None:
        folder, dry_run, ignore, refresh = _parse_cli_args(["prog", "/some/path"])
    ...
    def test_folder_with_dry_run(self) -> None:
        folder, dry_run, ignore, refresh = _parse_cli_args(["prog", "/path", "--dry-run"])
        assert folder == "/path"
    ...
    def test_folder_with_ignore_latest_date_check(self) -> None:
        folder, dry_run, ignore, refresh = _parse_cli_args(["prog", "/path", "--ignore-latest-date-check"])
        assert folder == "/path"
        assert dry_run is False
    ...
    def test_folder_with_refresh_accounts(self) -> None:
        folder, dry_run, ignore, refresh = _parse_cli_args(["prog", "/path", "--refresh-accounts"])
        assert folder == "/path"
        assert dry_run is False
        assert ignore is False
    ...
    def test_all_flags_after_folder(self) -> None:
        folder, dry_run, ignore, refresh = _parse_cli_args(
    ...
    def test_folder_after_flags(self) -> None:
        folder, dry_run, ignore, refresh = _parse_cli_args(["prog", "--dry-run", "/path"])
        assert folder == "/path"
        assert dry_run is True
...
class TestParseCLIArgsAllFlagCombinations:
    ...
    def test_flag_combination(
        ...
        folder, dry_run, ignore, refresh = _parse_cli_args(argv)
        assert folder == "/path"
...
class TestParseCLIArgsHypothesis:
    @given(
        ...
        flags=st.lists(
            st.sampled_from(["--dry-run", "--ignore-latest-date-check", "--refresh-accounts"]),
            unique=True,
        ),
    )
    def test_folder_always_extracted(self, folder: str, flags: list[str]) -> None:
        argv = ["prog"] + flags + [folder]
        result_folder, _, _, _ = _parse_cli_args(argv)
    ...
    def test_dry_run_flag_detection(self, folder: str) -> None:
        folder_val, dry, _, _ = _parse_cli_args(["prog", folder, "--dry-run"])
        assert dry is True
        _, no_dry, _, _ = _parse_cli_args(["prog", folder])
