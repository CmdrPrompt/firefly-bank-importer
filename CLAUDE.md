...
This project imports bank transactions from CSV exports (SEB, ICA, Nordea) into a
[Firefly III](https://www.firefly-iii.org/) instance via its REST API.
...
Tasks live in `docs/tasks/TASK-XXX-short-description.md`. See the Firefly Workflow
Guardian agent (`.github/agents/firefly-workflow-guardian.agent.md`) for the task file
format and full workflow enforcement.
...
# run /clear before starting the next task
```
...
Use the **Firefly Bug Triage** agent (`.github/agents/firefly-bug-triage.agent.md`) to
hunt for bugs without fixing anything. It analyses code against the requirements spec,
