...
- Added web UI account refresh via `POST /api/refresh-accounts` endpoint: triggers live account discovery from Firefly, updates the local cache, creates missing import folders, and returns a summary with total accounts and new folders created; index page includes a "Uppdatera konton" button (TASK-023).
