...
- Account discovery, transaction/transfer posting, opening-balance detection, and multi-folder import with cross-account transfer detection are now available as a documented, importable public interface (`firefly_bank_importer.service`, also re-exported from the top-level `firefly_bank_importer` package) with no dependency on any web framework or HTTP server, so an external application can drive imports against its own `FireflyClient` instance without depending on this repository's CLI. See `docs/SERVICE_LAYER_INTERFACE.md` for the interface guide (TASK-068).
