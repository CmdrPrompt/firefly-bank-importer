...
5. Update CHANGELOG.md with a concise behavior-first entry. This must happen **before** staging. Follow the style rules in the Changelog section of CLAUDE.md.
6. Ensure CHANGELOG.md is included on the `**Stage:**` line in the task file (or stage it explicitly with `git add CHANGELOG.md`) so it is not missed by `make stage-task`.
